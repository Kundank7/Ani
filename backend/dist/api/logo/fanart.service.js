...
exports.getTVDBIdFromAniList = getTVDBIdFromAniList;
exports.getFanartLogo = getFanartLogo;
exports.getAnimeLogo = getAnimeLogo;
exports.warmupAnimeDatabase = warmupAnimeDatabase;
exports.batchGetAnimeLogos = batchGetAnimeLogos;
...
const tvdbMappingCache = new Map();
...
function getOverrideLogo(anilistId) {
...
async function getTVDBIdFromAniList(anilistId) {
    ...
    try {
        ...
        if (!animeDatabase || (now - databaseLastFetched) > DATABASE_CACHE_TTL) {
            ...
            const response = await axios_1.default.get('https://raw.githubusercontent.com/Fribb/anime-lists/master/anime-list-mini.json', { timeout: 15000 });
        ...
        const entry = animeDatabase.find((item) => item.livechart_id === anilistId || item.anilist_id === anilistId);
 ...
 */
async function getFanartLogo(tvdbId) {
    // Check cache first
    ...
    try {
        const response = await axios_1.default.get(`${FANART_BASE_URL}/${tvdbId}`, {
 ...
 */
async function getAnimeLogo(anilistId) {
 ...
 */
async function warmupAnimeDatabase() {
    console.log('[Fanart] Pre-warming anime database...');
    try {
        const response = await axios_1.default.get('https://raw.githubusercontent.com/Fribb/anime-lists/master/anime-list-mini.json', { timeout: 30000 });
        if (response.data && Array.isArray(response.data)) {
 ...
 */
async function preWarmPopularLogos() {
 ...
 */
async function batchGetAnimeLogos(anilistIds) {
