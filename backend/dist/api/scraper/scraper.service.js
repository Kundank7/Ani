...
const animepahe_1 = require("../../scraper/animepahe");
const animekai_1 = require("../../scraper/animekai");
...
class ScraperService {
    ...
    cache = new Map();
    inFlight = new Map();
    hotStreamKeys = new Map();
    constructor() {
        this.fastScraper = new animepahe_1.AnimePaheScraper();
        this.animeKaiScraper = new animekai_1.AnimeKaiScraper();
    }
    isAnimePaheSession(session) {
        return /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(String(session || '').trim());
    }
    getEpisodeScraper(session) {
    ...
    async resolveAnimePaheStreamTarget(animeSession, epSession) {
        ...
        if (!this.isAnimePaheSession(resolvedAnimeSession))
            return null;
        ...
        const resolvedEpisode = Array.isArray(episodes?.episodes)
            ? episodes.episodes.find((episode) => Number(episode?.episodeNumber) === episodeNumber)
            : null;
        const resolvedEpSession = String(resolvedEpisode?.session || '').trim();
        if (!resolvedEpSession)
            return null;
        return { animeSession: resolvedAnimeSession, epSession: resolvedEpSession };
    }
    async getOrLoad(key, ttlMs, loader, options) {
        const now = Date.now();
        ...
        const redisCached = await (0, redis_cache_1.cacheGet)(key);
        if (redisCached !== null) {
        ...
        try {
            acquiredLock = await (0, redis_cache_1.acquireLock)(lockKey, lockTtlSeconds);
        }
        catch {
        ...
        if (!acquiredLock) {
            ...
            while (Date.now() - pollStart < pollTimeout) {
                ...
                const polled = await (0, redis_cache_1.cacheGet)(key);
                if (polled !== null) {
        ...
        const promise = loader()
            ...
            if (shouldCache) {
                ...
                (0, redis_cache_1.cacheSet)(key, value, Math.ceil(ttlMs / 1000)).catch((error) => {
            ...
            if (acquiredLock) {
                (0, redis_cache_1.releaseLock)(lockKey).catch(() => undefined);
            }
        });
    ...
    trackHotStream(animeSession, epSession) {
        ...
        if (this.hotStreamKeys.size > 200) {
            const sorted = [...this.hotStreamKeys.entries()].sort((a, b) => (a[1].hits * 1000000 + a[1].lastAccess) - (b[1].hits * 1000000 + b[1].lastAccess));
            for (let i = 0; i < sorted.length - 150; i++) {
                this.hotStreamKeys.delete(sorted[i][0]);
            }
        }
    }
    async fetchStreamLinksWithRetries(animeSession, epSession) {
    ...
    async searchAnimePahe(query) {
        const normalized = query.toLowerCase().trim();
        return this.getOrLoad(`search:animepahe:v1:${normalized}`, 5 * 60 * 1000, async () => {
            const results = await this.fastScraper.search(query);
            return Array.isArray(results)
                ? results.filter((item) => this.isAnimePaheSession(String(item?.session || '')))
                : [];
        }, {
    ...
    async getEpisodes(session) {
        const isCompleteEpisodePayload = (value) => {
        ...
        const waitForFullCache = async (timeoutMs) => {
            const startedAt = Date.now();
            while (Date.now() - startedAt < timeoutMs) {
                const waitedCache = await (0, redis_cache_1.cacheGet)(fullCacheKey);
        ...
        const fullCached = await (0, redis_cache_1.cacheGet)(fullCacheKey);
        ...
        return this.getOrLoad(shortCacheKey, shortTtlMs, async () => {
            let hasLock = await (0, redis_cache_1.acquireLock)(lockKey, 90);
            if (!hasLock) {
                const waitedCache = await waitForFullCache(25000);
                if (waitedCache)
                    return waitedCache;
                hasLock = await (0, redis_cache_1.acquireLock)(lockKey, 90);
            }
            if (!hasLock) {
                const staleCached = await (0, redis_cache_1.cacheGet)(shortCacheKey);
            ...
            try {
                ...
                if (Array.isArray(fast.episodes) && fast.episodes.length > 0) {
                    if (isCompleteEpisodePayload(fast)) {
                        (0, redis_cache_1.cacheSet)(fullCacheKey, fast, Math.ceil(fullTtlMs / 1000)).catch((error) => {
    ...
    async getStreams(animeSession, epSession) {
        ...
        return this.getOrLoad(key, 5 * 60 * 1000, async () => this.fetchStreamLinksWithRetries(target.animeSession, target.epSession), {
            shouldCache: (value) => Array.isArray(value) && value.length > 0,
            allowCached: (value) => Array.isArray(value) && value.length > 0,
        });
    }
    async prefetchStreams(animeSession, epSessions) {
    ...
    getHotStreamCandidates(limit = 20) {
        const now = Date.now();
