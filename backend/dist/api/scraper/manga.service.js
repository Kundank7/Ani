...
exports.prefetchChapters = prefetchChapters;
exports.getHotUpdates = getHotUpdates;
exports.getLatestManga = getLatestManga;
exports.getNewManga = getNewManga;
exports.getMangaDirectory = getMangaDirectory;
exports.getEnrichedSpotlight = getEnrichedSpotlight;
exports.warmSpotlightCache = warmSpotlightCache;
const mangakatana = __importStar(require("../../scraper/mangakatana"));
...
const hashKey = (input) => (0, crypto_1.createHash)('sha1').update(input).digest('hex');
...
const normalizeTitle = (value) => String(value || '')
    ...
    .replace(/\s+/g, ' ')
    .trim();
const getTitleCandidates = (media) => {
...
const scoreSearchResult = (candidate, titleCandidates) => {
...
async function resolveScraperIdFromAniList(anilistId, media) {
    const mapping = await mapping_service_1.mappingService.getMapping(anilistId).catch(() => null);
    if (mapping?.id) {
        return mapping.id;
    }
    ...
    for (const title of titleCandidates.slice(0, 6)) {
        ...
        const cached = await (0, redis_cache_1.cacheGet)(resolveCacheKey).catch(() => null);
        if (cached) {
            await mapping_service_1.mappingService.saveMapping(anilistId, cached, title).catch(() => undefined);
            return cached;
        }
        ...
        if (best && best.score >= 60) {
            await (0, redis_cache_1.cacheSet)(resolveCacheKey, best.item.id, 7 * 24 * 60 * 60).catch(() => undefined);
            await mapping_service_1.mappingService.saveMapping(anilistId, best.item.id, best.item.title || title).catch(() => undefined);
            return best.item.id;
        }
    }
    return null;
...
async function searchManga(query) {
    ...
    const redisCached = await (0, redis_cache_1.cacheGet)(redisKey).catch(() => null);
    ...
    if (results.length > 0) {
        ...
        (0, redis_cache_1.cacheSet)(redisKey, results, Math.ceil(SEARCH_CACHE_TTL / 1000)).catch((error) => {
...
async function getMangaDetails(id) {
    ...
    if (isAniListId) {
        ...
        const anilistData = await anilist_service_1.anilistService.getMangaById(parseInt(id, 10));
        if (!anilistData)
...
async function getChapterList(id) {
    ...
    if (/^\d+$/.test(realId)) {
        const mapping = await mapping_service_1.mappingService.getMapping(realId).catch(() => null);
        if (mapping?.id) {
    ...
    const redisCached = await (0, redis_cache_1.cacheGet)(redisKey);
    if (redisCached && redisCached.length > 0) {
    ...
    const fetchPromise = (async () => {
        try {
            ...
            if (normalized.length > 0) {
                ...
                (0, redis_cache_1.cacheSet)(redisKey, normalized, Math.ceil(CHAPTER_LIST_CACHE_TTL / 1000)).catch((error) => {
...
const chapterListInFlight = new Map();
    ...
    const fetchPromise = (async () => {
        try {
            ...
            if (pages && pages.length > 0) {
                ...
                (0, redis_cache_1.cacheSet)(redisKey, pages, Math.ceil(PAGES_CACHE_TTL / 1000)).catch((error) => {
 ...
 */
async function prefetchChapters(urls) {
...
async function getPagedScraperManga(page, scraperFunc) {
    ...
    const mapped = slicedResults.map((item) => {
...
async function getEnrichedSpotlight() {
    ...
    try {
        ...
        let topManga = [];
        try {
            const trending = await anilist_service_1.anilistService.getTrendingManga(1, 10);
        ...
        if (topManga.length === 0) {
            ...
            const fallbackManga = hotUpdates.slice(0, 10).map((update) => ({
 ...
 */
async function enrichAndCache(mangaList) {
 ...
 */
async function warmSpotlightCache() {
 ...
 */
async function enrichWithAniListPhotos(mangaList) {
    ...
    const enriched = await Promise.all(mangaList.map(async (item) => {
        try {
            ...
            const cached = await (0, redis_cache_1.cacheGet)(cacheKey);
            if (cached) {
                return { ...item, thumbnail: cached, coverImage: cached };
            }
            const searchResults = await anilist_service_1.anilistService.searchManga(item.title, 1, 1);
            if (searchResults.media && searchResults.media.length > 0) {
                ...
                if (photo) {
                    ...
                    await (0, redis_cache_1.cacheSet)(cacheKey, photo, 7 * 24 * 60 * 60);
                    return {
                        ...item,
                        thumbnail: photo,
                        coverImage: photo,
